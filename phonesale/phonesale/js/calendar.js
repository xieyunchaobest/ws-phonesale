...
function writeIframe()
{
    ...
    "    <td width=60 id=meizzYearHead  title='点击此处选择年份' bgColor='#4682B4' onclick='parent.funYearSelect(parseInt(this.innerText, 10))'"+
    "></td>"+
    "    <td width=50 id=meizzYearMonth title='点击此处选择月份' bgColor='#4682B4' onclick='parent.funMonthSelect(parseInt(this.innerText, 10))'"+
    "></td>"+
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         strIframe +="<td class=out title='关闭日历,快捷键:[Esc]'><input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 4px; width: 100%; height: 100%; border: 0' onfocus='this.blur()'"+
         " type=button value='C' onclick='parent.hiddenCalendar()'></td>"
		 +"<td colspan=2 class=out title='清除文本框中日期'><input style=' background-color: "+
         WebCalendar.btnBgColor +";cursor: hand; padding-top: 4px; width: 100%; height: 100%; border: 0' onfocus='this.blur()'"+
         " type=button value='清除' onclick='parent.clear();parent.hiddenCalendar();'></td></tr></table>"+
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.darkColor  = "#4682B4";    //控件的暗色
    this.lightColor = "#ddccff";    //控件的亮色
    this.btnBgColor = "#4682B4";    //控件的按钮背景色
    this.wordColor  = "#ddccff";    //控件的文字颜色
    this.wordDark   = "#aa99ff";    //控件的暗文字颜色
    this.dayBgColor = "#4682B4";    //日期数字背景色
    this.todayColor = "#aa99ff";    //今天在日历上的标示背景色
    this.DarkBorder = "#5599bb";    //日期显示的立体表达色
...
function funYearSelect() //年份的下拉框
{
    ...
    var min = 1990;//(y - n >= 1000) ? y - n : 1000;
    var max = 2100;//(y + n <= 9999) ? y + n : 9999;
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        hiddenCalendar();
		document.getElementById(srcName).onchange();
	}
}
